...
<body>
	...
	<form method="POST" action="${contextRoot}/app/users/do/${commandName}">
		...
		<fieldset>
			...
			<div>
				<em>Caution</em>: every user must have at least one role; also, there must always be at least one user with the
				"v-admin" role.
			</div>
			<table>
				<c:forEach var="role" items="${userRoles}">
					<tr>
						<td><input type="checkbox" name="role-${role.name}" id="${role.name}"></td>
